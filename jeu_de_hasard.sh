...
while true; do
	...
	if [[ "$nbre_utilisateur" -eq "$nbre_a_deviner" ]]; then
		#si le joueur à trouver on l'affiche et on sort du boucle(fin du jeu)
	...
	if (( "$essaie" == "5" || "$essaie" == "10" )); then
		...
		read -p " Entrez votre reponse : " reponse
		if [[ "$reponse" == "2" ]]; then
